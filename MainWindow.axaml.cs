    ...
    private void UpdateAudioList(JsonElement formats)
    {
        ...
        foreach (var fmt in formats.EnumerateArray())
        {
            ...
            var candidate = new AudioCandidate
            {
                ...
                Abr = isAudioOnly ? (100000 + abr) : (height > 0 ? height : abr),
                Height = height,
        ...
        foreach (var fmt in formats.Value.EnumerateArray())
{
    ...
    {
        (2160, "4K / Лучшее доступное"),
        (1440, "2K / Лучшее доступное"),
        (1080, "1080p (FHD)"),
        (720,  "720p (HD)"),
        (480,  "480p (SD)"),
        (360,  "360p (SD)"),
...
private void ShowDownloadStartToast(string filename)
{
    ...
    var builder = new ToastContentBuilder()
        .AddText($"Загрузка: {filename.Trim()}")  // единственная строка сверху
...
private void ShowDownloadCompleteToast(string filename, bool success, string message = "")
{
    ...
    if (success)
    {
        builder.AddText("✅ Загрузка завершена")
               .AddText(filename);
    }
    else
    {
        builder.AddText("❌ Ошибка загрузки")
               .AddText(message);
    }
...
Avalonia.Threading.Dispatcher.UIThread.Post(() =>
{
    ...
    catch (Exception ex)
    {
     ...
     {
         ShowInlineNotification("Перевод", "Перевожу название видео...", 0);
         ...
         // Обновляем заголовок в интерфейсе сразу, чтобы пользователь видел результат
         Avalonia.Threading.Dispatcher.UIThread.Post(() => VideoTitle.Text = finalFileName);
     }
...
string langHeader = !string.IsNullOrEmpty(targetLang) ? $"--extractor-args \"youtube:lang={targetLang}\"" : "";
    string formatArg = "";
    ...
    if (isAudioMode)
    {
        ...
        {
            var muxedId = PickMuxedFormatId(targetLang, audioNoteKey, 0);
            if (!string.IsNullOrWhiteSpace(muxedId)) audioId = muxedId;
        }
        formatArg = $"-f {audioId}";
        conversionArgs = $"-x --audio-format mp3 --audio-quality {bitrateStr}k";
    }
    else // Режим видео
    {
        string? qualityLabel = QualityOrBitrateList.SelectedItem?.ToString() ?? "";
        
        // Надежно получаем число разрешения (1080, 720 и т.д.)
        string height = Regex.Match(qualityLabel, @"\d+").Value;
        if (string.IsNullOrEmpty(height)) height = "1080"; 
        ...
        if (audioSource == "muxed")
        {
            ...
            if (string.IsNullOrWhiteSpace(muxedId)) muxedId = audioId;
        }
        ...
        if (hasFfmpeg)
        {
            ...
            // Если выбрана конкретная дорожка (например, при выборе языка), используем её
            if (!string.IsNullOrEmpty(safeAudioId) && safeAudioId != "bestaudio")
            {
                formatArg = $"-f \"bestvideo[height<={height}]+{safeAudioId}/bestaudio/best\"";
            }
            else
            {
                // Иначе просто лучшее видео до указанной высоты + лучшее аудио
                formatArg = $"-f \"bestvideo[height<={height}]+bestaudio/best\"";
            }
        ...
        else
        {
            ...
            {
            formatArg = $"-f \"best[height<={height}][ext=mp4]/best\"";
            conversionArgs = "--recode-video mp4";
            }
        }

// И далее при создании процесса для скачивания:
/* if (false)
{
var psi = new ProcessStartInfo
{
    ...
    WorkingDirectory = AppDirectory,
        ...
        if (hasFfmpeg)
        {
            ...
            {
            // Если пользователь выбрал конкретную аудиодорожку (audioId != "bestaudio"), предпочитаем её при слиянии
            var safeAudioId = (audioId ?? "").Replace("\"", "").Replace("'", "");
            if (!string.IsNullOrEmpty(safeAudioId) && safeAudioId != "bestaudio")
            {
                // Предпочитаем выбранную дорожку (в основном это 140-* в текущей реализации)
                formatArg = $"-f \"bestvideo[height<={height}][vcodec^=avc1][ext=mp4]+{safeAudioId}[ext=m4a]/bestaudio[ext=m4a]/bestaudio/best\"";
            }
            else
            {
                // Предпочитаем m4a (mp4-совместимое) аудио, иначе fallback на любое доступное bestaudio
                formatArg = $"-f \"bestvideo[height<={height}][vcodec^=avc1][ext=mp4]+bestaudio[ext=m4a]/bestaudio/best\"";
            }

            conversionArgs = "--merge-output-format mp4 --recode-video mp4";
            }
        }
        else
        {
            Log("W", "ffmpeg не найден — выбрана безопасная стратегия скачивания (встроенный формат с аудио)");
            formatArg = $"-f \"best[height<={height}][ext=mp4]/best\"";
            conversionArgs = "--recode-video mp4";
        }
    }
